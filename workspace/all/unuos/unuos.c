...
static Array* getRoot(void) {
	...
	// UnuOS settings entry (virtual — opens an in-minui settings menu)
...
static void readyResumePath(char* rom_path, int type) {
	...
	sprintf(slot_path, "%s/.unuos/%s/%s.txt", SHARED_USERDATA_PATH, emu_name, rom_file); // /.userdata/.unuos/<EMU>/<romname>.ext.txt
...
static void openRom(char* path, char* last) {
	...
	if (should_resume) {
		...
		if (has_m3u) {
			...
			sprintf(disc_path_path, "%s/.unuos/%s/%s.%s.txt", SHARED_USERDATA_PATH, emu_name, rom_file, slot); // /.userdata/arm-480/.unuos/<EMU>/<romname>.ext.0.txt
...
// UnuOS settings screen (launcher-side, no game required).
